...
static int8_t  USB_AudioPlaybackSessionCallback(AUDIO_SessionEvent_t event, AUDIO_Node_t* node_handle, struct AUDIO_Session* session_handle);
...
{
	if((play_session->session.state == AUDIO_SESSION_INITIALIZED) || (play_session->session.state == AUDIO_SESSION_STOPPED))
	{
...
#endif /*USE_AUDIO_USB_INTERRUPT*/
/**
 * @brief  Session callback for the audio playback, it receives events from nodes.
 * @param  event(IN):
